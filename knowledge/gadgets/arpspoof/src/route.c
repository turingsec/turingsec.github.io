...
int get_local_info(int rsock, struct ifcfg *ifcfg)
{
	...
	ifcfg->if_index = ifr.ifr_ifindex;
	
	ifcfg->ipa.s_addr = ntohl(ifcfg->ipa.s_addr);
	ifcfg->bcast.s_addr = ntohl(ifcfg->bcast.s_addr);
	ifcfg->nmask.s_addr = ntohl(ifcfg->nmask.s_addr);
	ifcfg->ip_subnet = ifcfg->ipa.s_addr & ifcfg->nmask.s_addr;
